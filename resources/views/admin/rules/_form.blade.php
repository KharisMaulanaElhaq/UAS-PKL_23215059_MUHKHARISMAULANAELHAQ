...
@php
    ...
    $filterUrl = $filterAction ?? url()->current();
@endphp

<div class="row g-3 align-items-end">
    <div class="col-md-6 col-lg-3">
        <label class="form-label" for="komoditasFilterSelect">Komoditas</label>
        <select id="komoditasFilterSelect" class="form-select">
    ...
    <div class="col-12">
        @if ($komoditasId)
            <span class="small text-muted">
                ...
                {{ $gejalaList->count() }} gejala · {{ $penyakitList->count() }} penyakit · {{ $hamaList->count() }} hama
            </span>
        @else
            <span class="small text-warning">
                <i class="ti ti-info-circle me-1"></i>
                Pilih komoditas untuk memfilter daftar gejala, penyakit, dan hama.
            </span>
        @endif
    </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center text-muted py-4">
                                    Pilih komoditas terlebih dahulu untuk menampilkan gejala.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
@push('scripts')
    <script>
        (function () {
            const filterUrl = @json($filterUrl);
            const komoditasSelect = document.getElementById('komoditasFilterSelect');
            ...
            if (komoditasSelect) {
                komoditasSelect.addEventListener('change', function () {
                    ...
                    if (this.value) {
                        url.searchParams.set('komoditas_id', this.value);
                    } else {
