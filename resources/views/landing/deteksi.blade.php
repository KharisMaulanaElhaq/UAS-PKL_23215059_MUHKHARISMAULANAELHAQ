...
@section('title', 'Deteksi ' . $komoditas->nama)

@section('content')
    <section class="deteksi-section lp-photo-soft">
        <div class="container">
            <div class="mb-3 pt-2">
                <a href="{{ route('deteksi') }}" class="btn btn-sm btn-outline-warning rounded-pill">
                    <i class="ti ti-arrow-left me-1"></i> Ganti Komoditas
                </a>
            </div>
            <div class="row justify-content-center">
                <div class="col-lg-10 col-xl-9">
                    <div class="deteksi-card" id="deteksiApp" data-komoditas-slug="{{ $komoditas->slug }}">
                        <div class="deteksi-header">
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    <h2 class="h4 fw-bold mb-1">
                                        <i class="ti ti-stethoscope me-2"></i>Deteksi {{ $komoditas->nama }}
                                    </h2>
                                ...
                                @guest
                                    <span class="badge bg-white text-warning"><i class="ti ti-user me-1"></i>Mode Tamu</span>
                                @elseif(auth()->user()->isPengguna())
                                    <span class="badge bg-white text-warning"><i class="ti ti-user-check me-1"></i>{{ auth()->user()->name }}</span>
                                @else
                                    <span class="badge bg-white text-warning"><i class="ti ti-shield-check me-1"></i>{{ auth()->user()->name }}</span>
                                @endguest
                            </div>
                        ...
                        <div class="deteksi-actions" id="actionBar">
                            ...
                            <button type="button" class="btn btn-warning rounded-pill d-none" id="btnFinish">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-4 border-0">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title fw-bold"><i class="ti ti-adjustments me-2 text-warning"></i>Tingkat Keyakinan</h5>
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mt-4">
                        <button type="button" class="btn btn-warning rounded-pill w-100" id="btnConfirmCf">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-4 border-0">
                <div class="modal-body p-4 text-center">
                    ...
                    <button type="button" class="btn btn-warning rounded-pill px-4" id="btnGuestContinue">
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const apiBase = '{{ url('/deteksi/' . $komoditas->slug) }}';
                ...
                mainArea.innerHTML = `
                    <div class="target-filter px-4 pt-3">
                        <div class="btn-group target-filter-tabs" role="group">
                            <button type="button" class="btn btn-sm ${targetFilter === 'all' ? 'btn-warning' : 'btn-outline-warning'} rounded-pill" data-filter="all">Semua</button>
                            <button type="button" class="btn btn-sm ${targetFilter === 'penyakit' ? 'btn-warning' : 'btn-outline-warning'} rounded-pill" data-filter="penyakit">
                                <i class="ti ti-virus me-1"></i>Penyakit
                            </button>
                            <button type="button" class="btn btn-sm ${targetFilter === 'hama' ? 'btn-warning' : 'btn-outline-warning'} rounded-pill" data-filter="hama">
