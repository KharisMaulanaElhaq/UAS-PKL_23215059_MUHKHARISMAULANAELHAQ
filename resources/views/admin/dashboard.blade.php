...
@push('styles')
<style>
    /* Custom Brown color for Total Deteksi card */
    .bg-brown { background-color: rgba(180, 83, 9, var(--bs-bg-opacity, 1)) !important; }
    .text-brown { color: #b45309 !important; }
    .border-brown { border-color: rgba(180, 83, 9, var(--bs-border-opacity, 1)) !important; }
</style>
@endpush
...
@section('content')
    <x-page-header
        title="Dashboard"
        subtitle="Ringkasan SIPATAN — Tanaman Jagung">
        <x-slot:actions>
            <form method="GET" class="d-flex gap-2">
                <select name="komoditas_id" class="form-select form-select-sm" onchange="this.form.submit()">
                    <option value="">Semua Komoditas</option>
                    @foreach ($komoditasList as $item)
                        <option value="{{ $item->id }}" @selected((int) $komoditasId === $item->id)>{{ $item->nama }}</option>
                    @endforeach
                </select>
            </form>
        </x-slot:actions>
            ...
            <div class="card h-100">
                <div class="card-header bg-white d-flex justify-content-between align-items-center px-4 py-3">
                    ...
                    <div class="btn-group btn-group-sm" role="group" id="deteksiChartToggle">
                        <button type="button" class="btn btn-outline-warning active" data-target="deteksi_penyakit">Penyakit</button>
                        <button type="button" class="btn btn-outline-warning" data-target="deteksi_hama">Hama</button>
                    </div>
                </div>
                <div class="card-body p-4">
            ...
            waitForApex(function () {
                ...
                function buildRuleOptions(target) {
                    ...
                    return {
                        ...
                        colors: [target === 'penyakit' ? '#d97706' : '#f59e0b'],
                        plotOptions: {
                            bar: {
                                borderRadius: 6,
                ...
                const overviewChart = new ApexCharts(overviewEl, {
                    ...
                    colors: ['#f59e0b', '#FB2C36', '#F0B100', '#b45309'],
                ...
                if (deteksiTargetEl) {
                    ...
                    function buildDeteksiTargetOptions(key) {
                        ...
                        return {
                            ...
                            colors: ['#d97706', '#f59e0b', '#fbbf24', '#fcd34d', '#fde68a', '#b45309'],
