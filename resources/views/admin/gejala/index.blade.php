...
@section('content')
    <x-page-header
        title="Data Gejala"
        subtitle="Kelola gejala per komoditas — satu gejala dapat dipakai beberapa penyakit/hama dengan bobot CF berbeda"
    ...
    <div class="card">
        <div class="card-header bg-white px-4 py-3">
            <x-komoditas-filter
                ...
                :action="route('gejala.index')" />

            <form method="GET" action="{{ route('gejala.index') }}" class="row g-2 align-items-center mt-2">
                @if ($komoditasId)
                ...
                <div class="col-md-auto col-12 d-flex gap-2">
                    ...
                    @if ($search !== '' || $komoditasId)
                        <a href="{{ route('gejala.index', $komoditasId ? ['komoditas_id' => $komoditasId] : []) }}" class="btn btn-light">Reset</a>
                    @endif
                </div>
            </form>
        </div>
                ...
                <tbody>
                    @forelse ($gejala as $item)
                        <tr>
                            ...
                            <td><span class="badge bg-secondary-subtle text-secondary">{{ $item->komoditas?->nama ?? '—' }}</span></td>
