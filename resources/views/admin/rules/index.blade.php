...
@section('content')
    <x-page-header
        ...
        <x-slot:actions>
            <a href="{{ route('rules.export', array_filter(['komoditas_id' => $komoditasId ?? null, 'jenis' => $jenis ?? null])) }}"
                class="btn btn-outline-success">
                <i class="ti ti-file-spreadsheet me-1"></i> Export Excel
            </a>
            <a href="{{ route('rules.create', array_filter(['komoditas_id' => $komoditasId ?? null])) }}" class="btn btn-primary">
    ...
    <div class="card">
        <div class="card-header bg-white px-4 py-3">
            <x-komoditas-filter
                :komoditas-list="$komoditasList"
                :komoditas-id="$komoditasId"
                :action="route('rules.index')" />

            <form method="GET" action="{{ route('rules.index') }}" class="d-flex gap-2 flex-wrap align-items-center mt-2">
                @if ($komoditasId)
                    <input type="hidden" name="komoditas_id" value="{{ $komoditasId }}">
                @endif
                ...
                @if ($jenis || $komoditasId)
                    <x-action-btn type="reset" :href="route('rules.index', array_filter(['komoditas_id' => $komoditasId]))" title="Reset filter" />
                @endif
            </form>
        </div>
            ...
            <table class="table align-middle mb-0">
                <thead>
                    <tr>
                        ...
                        <th style="width: 130px;">Komoditas</th>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-secondary-subtle text-secondary">
                                    {{ $rule->targetModel?->komoditas?->nama ?? '—' }}
                                </span>
                            </td>
                            <td>
