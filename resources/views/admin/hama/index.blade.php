    ...
    <div class="card">
        <div class="card-header bg-white px-4 py-3">
            <x-komoditas-filter
                ...
                :action="route('hama.index')" />

            <form method="GET" action="{{ route('hama.index') }}" class="row g-2 align-items-center mt-2">
                @if ($komoditasId)
                ...
                <div class="col-md-auto col-12 d-flex gap-2">
                    ...
                    @if ($search !== '' || $komoditasId)
                        <a href="{{ route('hama.index', $komoditasId ? ['komoditas_id' => $komoditasId] : []) }}" class="btn btn-light">Reset</a>
                    @endif
                </div>
            </form>
        </div>
